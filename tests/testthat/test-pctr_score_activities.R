test_that("with minimal data outptus a tibble", {
  ...
  out <- pctr_at_product_level(data)
  expect_s3_class(out, "tbl_df")
})

test_that("with 0-row data returns 0-row tibble", {
  data <- slice(pctr_ecoinvent_co2, 1)
  out <- pctr_at_product_level(data[0, ])
...
test_that("without crucial data errors gracefully", {
  expect_error(pctr_at_product_level(), "missing")
})
...
test_that("without crucial columns errors gracefully", {
  data <- pctr_ecoinvent_co2 |>
    slice(1) |>
    select(-"sec")
  expect_error(pctr_at_product_level(data), "sec.*not found")

  data <- pctr_ecoinvent_co2 |>
    slice(1) |>
    select(-"unit")
  expect_error(pctr_at_product_level(data), "unit.*not found")

  data <- pctr_ecoinvent_co2 |>
    slice(1) |>
    select(-"co2_footprint")
  expect_error(pctr_at_product_level(data), "co2_footprint.*not found")
})

test_that("is sensitive to low_threshold with 2 or more input rows but not 1", {
  data <- pctr_ecoinvent_co2 |> slice(1:2)
  out1 <- pctr_at_product_level(data, low_threshold = .1)
  out2 <- pctr_at_product_level(data, low_threshold = .9)
...
test_that("is sensitive to high_threshold with 2 or more input rows but not 1", {
  data <- pctr_ecoinvent_co2 |> slice(1:2)
  out1 <- pctr_at_product_level(data, high_threshold = .1)
  out2 <- pctr_at_product_level(data, high_threshold = .9)
  expect_false(identical(out1, out2))

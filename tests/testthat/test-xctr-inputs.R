...
test_that("is sensitive to low_threshold", {
  ...
  out1 <- xctr_product(companies, inputs, low_threshold = .1) |>
    xctr_company()
  out2 <- xctr_product(companies, inputs, low_threshold = .9) |>
    xctr_company()
...
test_that("is sensitive to high_threshold", {
  companies <- slice(companies, 1)
  inputs <- slice(inputs, 1:2)
  out1 <- xctr_product(companies, inputs, high_threshold = .1) |>
    xctr_company()
  out2 <- xctr_product(companies, inputs, high_threshold = .9) |>
    xctr_company()
  expect_false(identical(out1, out2))
})
...
test_that("values sum 1", {
  ...
  out <- xctr_product(companies, co2) |>
    xctr_company()

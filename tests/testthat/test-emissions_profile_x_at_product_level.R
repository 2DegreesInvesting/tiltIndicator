test_that("returns visibly (#238)", {
  ...
  expect_visible(emissions_profile_x_at_product_level(companies, inputs))
  products <- slice(products, 1)
  expect_visible(emissions_profile_x_at_product_level(companies, products))
})
...
test_that("outputs expected columns at product level", {
  ...
  out <- emissions_profile_x_at_product_level(companies, inputs)
...
test_that("with duplicated co2 throws no error (#435)", {
  ...
  expect_no_error(emissions_profile_x_at_product_level(companies, co2))
})
...
test_that("if `companies` lacks crucial columns, errors gracefully", {
  ...
  expect_error(emissions_profile_x_at_product_level(bad, co2), crucial)
...
test_that("if `co2` lacks crucial columns, errors gracefully", {
  ...
  expect_error(emissions_profile_x_at_product_level(companies, bad), crucial)
...
test_that("if the 'isic' column isn't a character, throws an error (#233)", {
  ...
  expect_error(emissions_profile_x_at_product_level(companies, co2), "must be.*character")
})

test_that("a 0-row `co2` yields an error", {
  expect_error(
    emissions_profile_x_at_product_level(companies[0L, ], products),
...
test_that("a 0-row `co2` yields an error", {
  expect_error(
    emissions_profile_x_at_product_level(slice(companies, 1), products[0L, ]),
...
test_that("a 0-row `companies` yields an error", {
  expect_error(
    emissions_profile_x_at_product_level(companies[0L, ], inputs),
    "companies.*can't have 0-row"
  )
})
...
test_that("a 0-row `inputs` yields an error", {
  expect_error(
    emissions_profile_x_at_product_level(slice(companies, 1), inputs[0L, ]),
    "co2.*can't have 0-row"
  )
})
...
test_that("if `companies` lacks crucial columns, errors gracefully", {
  ...
  crucial <- "activity_uuid_product_uuid"
  bad <- select(companies, -all_of(crucial))
  expect_error(emissions_profile_x_at_product_level(bad, inputs), crucial)
...
test_that("with a missing value in the co2* column errors gracefully", {
  ...
  expect_error(emissions_profile_x_at_product_level(companies, inputs), "co2_footprint")
})

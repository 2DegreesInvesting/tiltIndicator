test_that("outputs expected columns at product level", {
  ...
  out <- pstr_product(companies, scenarios)
...
test_that("with duplicated scenarios throws no error (#435)", {
  ...
  expect_no_error(pstr_product(companies, scenarios))
})
...
test_that("if `companies` lacks crucial columns, errors gracefully", {
  ...
  expect_error(pstr_product(bad, scenarios), crucial)
...
test_that("if `scenarios` lacks crucial columns, errors gracefully", {
  ...
  expect_error(pstr_product(companies, bad), crucial)
...
test_that("grouped_by includes the type of scenario", {
  ...
  product <- pstr_product(companies, co2)
...
test_that("error if a `type` has all `NA` in `sector` & `subsector` (#310)", {
  ...
  expect_error(pstr_product(companies, scenarios), "sector.*subsector.*type")
})

test_that("a 0-row `companies` yields an error", {
  expect_error(
    pstr_product(pstr_companies[0L, ], xstr_scenarios),
...
test_that("a 0-row `scenarios` yields an error", {
  expect_error(
    pstr_product(slice(pstr_companies, 1), xstr_scenarios[0L, ]),
...
test_that("with ';' in `*sector` throws an warning", {
  ...
  expect_snapshot_warning(pstr_product(companies, scenarios))
})

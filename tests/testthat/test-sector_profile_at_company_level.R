...
test_that("thresholds yield expected low, medium, and high risk categories", {
  ...
  default_low_mid <- 1 / 3
  product <- pstr_product(companies, mutate(scenarios, reductions = default_low_mid))
  out <- pstr_company(product)
  ...
  product <- pstr_product(companies, mutate(scenarios, reductions = above_default_low_mid))
  out <- pstr_company(product)
  ...
  product <- pstr_product(companies, mutate(scenarios, reductions = default_mid_high))
  out <- pstr_company(product)
  expect_equal(0, filter(out, risk_category == "low")$value)
  expect_equal(1, filter(out, risk_category == "medium")$value)
  expect_equal(0, filter(out, risk_category == "high")$value)
  ...
  product <- pstr_product(companies, mutate(scenarios, reductions = above_default_mid_high))
  out <- pstr_company(product)
  expect_equal(0, filter(out, risk_category == "low")$value)
  expect_equal(0, filter(out, risk_category == "medium")$value)
  ...
  below_0 <- -0.001
  product <- pstr_product(companies, mutate(scenarios, reductions = below_0))
  out <- pstr_company(product)
  expect_equal(1, filter(out, risk_category == "low")$value)
  expect_equal(0, filter(out, risk_category == "medium")$value)
  expect_equal(0, filter(out, risk_category == "high")$value)

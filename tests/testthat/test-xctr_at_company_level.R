...
test_that("outputs expected columns at company level", {
  ...
  product <- emissions_x_at_product_level(companies, co2)
...
test_that("is sensitive to low_threshold", {
  ...
  out1 <- emissions_x_at_product_level(companies, co2, low_threshold = .1) |>
    any_indicator_at_company_level()
  out2 <- emissions_x_at_product_level(companies, co2, low_threshold = .9) |>
    any_indicator_at_company_level()
...
test_that("is sensitive to high_threshold", {
  companies <- slice(companies, 1:2)
  co2 <- slice(products, 1:2)
  out1 <- emissions_x_at_product_level(companies, co2, high_threshold = .1) |>
    any_indicator_at_company_level()
  out2 <- emissions_x_at_product_level(companies, co2, high_threshold = .9) |>
    any_indicator_at_company_level()
  expect_false(identical(out1, out2))
})
...
test_that("for a company with 3 products of varying footprints, value is 1/3 (#243)", {
  ...
  out <- emissions_x_at_product_level(companies, co2, low_threshold, high_threshold) |>
    any_indicator_at_company_level()

test_that("outputs expected columns at product level", {
  ...
  inputs <- istr_inputs
  out <- spui_product(companies, scenarios, inputs)
...
test_that("with duplicated scenarios throws no error (#435)", {
  ...
  expect_no_error(spui_product(companies, scenarios, inputs))
})
...
test_that("if `companies` lacks crucial columns, errors gracefully", {
  ...
  expect_error(spui_product(bad, scenarios, inputs), crucial)
...
test_that("if `scenarios` lacks crucial columns, errors gracefully", {
  ...
  expect_error(spui_product(companies, bad, inputs), crucial)
...
test_that("if `inputs` lacks crucial columns, errors gracefully", {
  ...
  expect_error(spui_product(companies, scenarios, bad), crucial)
...
test_that("error if a `type` has all `NA` in `sector` & `subsector` (#310)", {
  ...
  expect_error(spui_product(companies, scenarios, inputs), "sector.*subsector.*type")
})

test_that("a 0-row `companies` yields an error", {
  expect_error(
    spui_product(istr_companies[0L, ], xstr_scenarios, istr_inputs),
...
test_that("a 0-row `scenarios` yields an error", {
  expect_error(
    spui_product(istr_companies, xstr_scenarios[0L, ], istr_inputs),

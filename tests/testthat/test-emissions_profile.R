test_that("wraps the output at product and company levels", {
  ...
  expect_equal(product, emissions_profile_x_at_product_level(companies, products))
...
test_that("*upstream() wraps the output at product and company levels", {
  ...
  product <- unnest_product(out)
  expect_equal(product, emissions_profile_x_at_product_level(companies, inputs))

...
#' }
#' @noRd
pctr_company_scores_old <- function(path) {
  # TODO: Refactor
  # Tests run privately until we have public toy data
  wrap_rmd(path)[["company_scores"]]
}

#' @param data_ecoinvent FIXME:
#'   * Source: FIXME "input_data/ecoinvent_activities.csv" via Tilman
#' @param data_companies FIXME: Sample data:
#'   * Source: FIXME Tilman at "input_data/MVP_sample_dataset.csv".
#'   * Public or private?
#'   * Can we generate it in data-raw/?
#'
#' @examples
#' \dontrun{
#' library(readr)
...
#' }
#' @noRd
pctr_company_scores <- function(data_ecoinvent, data_companies) {
  out <- pctr_impl(
    data_ecoinvent = data_ecoinvent,
    data_companies = data_companies
  )
  out[["company_scores"]]
}

pctr_impl <- function(data_ecoinvent, data_companies) {
  # FIXME: Remove
  set.seed(123)
  ...
  out <- list(
    # FIXME: Extract plots into their own functions and call barplot()
    plot_score_all = prop.table(table(data_short$score_all)),
    plot_score_unit = prop.table(table(data_short$score_unit)),
    plot_score_unit_isic = prop.table(table(data_short$score_unit_isic))
  )
  ...
  out <- append(out, list(company_scores = company_scores))
  out
}

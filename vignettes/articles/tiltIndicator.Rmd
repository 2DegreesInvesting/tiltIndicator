...
```{r child=tiltIndicator:::template_to_rmd("example-emissions-profile.R")}
```
...
```{r child=extdata_path("child/example-emissions-profile-upstream.Rmd")}
```
...
result_upstream <- emissions_profile_upstream(companies, inputs)
result_upstream

...
If the number of files is too large you may need to combine them gradually.

### Combine gradually

Consider the files at company level. We can put all file paths in a table and
create a column to group them in any number of chunks.
...
# Each chunk should have no more files than you can successfully read
chunked |> count(chunk)
...
And now the number of chunk files should be small enough to read at once.
```{r}

...
This is done based on the input products' carbon footprints compared to 4
...
This is done based on their position within the distribution of co2 footprints
...
barplot(prop.table(table(ecoinvent_scores$score_sector)))
...
## 3.4 Vizualisation example

Let us plot the result for the company id
"fleischerei-stiefsohn_00000005219477-001". The score aggregated is the
percentage of products of that company that are based on their carbon footprint
in either low, medium or high transition risk, relatively 4 different benchmarks
(all input products, input products with same unit, input products with same
input sector, and input products with same unit and input sector). To do: Create
a function that shows the scores per benchmark for one example company, as Linda
did in her pstr mvp.

```{r}
pstr_plot_company <- function(data, company_id) {
  level_order <- c("low", "medium", "high")

  data |>
    filter(.data$company_id == .env$company_id) |>
    ggplot(aes(x = .data$score, y = .data$share_all)) +
    scale_x_discrete(limits = level_order) +
    geom_col()
}
# plot for `share_all` column only (plot function will be updated later)
pstr_plot_company(pctr_output, "fleischerei-stiefsohn_00000005219477-001")
```

## 3.5 write csv into output folder

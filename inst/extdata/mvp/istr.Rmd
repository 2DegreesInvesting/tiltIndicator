...
#' Reshape wide dataframe companies to long data frame
#' @param companies A [data.frame] like [raw_istr_companies].
#' 
#' @family ISTR functions
#' 
#'@return A dataframe with:
#'    * Long data frame of `companies` dataset.
#'    * One column for Ecoinvent input products.
#'    * One column for the corresponding Ecoinvent Sectors to the input products.
istr_reshaping <- function(companies) {
  reshape(
  companies,
  direction = "long",
  varying = list(
...
```{r}
raw_istr_companies <- read.csv(here("data-raw", "istr_companies.csv"), sep = ";")
glimpse(raw_istr_companies)
```

```{r}
istr_ep_weo <- readxl::read_xlsx((here('data/istr_ep_weo.xlsx')))
glimpse(istr_ep_weo)
```

```{r}
istr_weo_2022 <- readr::read_csv(here('data/istr_weo_2022.csv'), show_col_types = FALSE)
glimpse(istr_weo_2022)
```

# Data Preperation
```{r}
reshaped_istr_companies <- istr_reshaping(raw_istr_companies)

reshaped_istr_companies |> tibble::as_tibble()
```
...
istr_companies |> tibble::as_tibble()
```
...
with_reductions |> tibble::as_tibble()
```
...
with_transition_risk |> tibble::as_tibble()
```

...
#'
#' @examples
add_reductions <- function(companies, ep_weo, weo_2022) {
  companies |>
...
with_reductions <- add_reductions(companies, ep_weo, weo_2022)
...
#' Title
#'
#' @param data A data frame. The ouptput of [add_reductions()].
#'
#' @return
#' @export
#'
#' @examples
add_transition_risk <- function(data) {
  data |>
    mutate(
...
with_rt <- add_transition_risk(with_reductions)
...
with_transition_risk2 <- with_rt |>
  left_join(n_products_per_companies, by = "company_name")

with_score_aggregated <- with_transition_risk2 |>

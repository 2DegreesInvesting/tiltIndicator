...
pstr_path <- function(...) {
  fs::path_home("Downloads", "tilt", "tiltIndicator", "pstr", ...)
}
here <- pstr_path
here() |> fs::dir_tree()
```
...
final_df |> write_csv("with_reductions.csv")
```
...
final_df_cat |> write_csv("with_transition_risk.csv")
```
...
aggregated_score_df |> write_csv("with_score_aggregated.csv")
```

...
if (is.null(params$input1)) {
  data_ecoinvent <- readr::read_csv(tiltIndicator:::pctr_path("input_data", "ecoinvent_activities.csv"))
} else {
...
if (is.null(params$input1)) {
  data_companies <- readr::read_csv(tiltIndicator:::pctr_path("input_data", "MVP_sample_dataset.csv"))
} else {
...
readr::write_csv(company_scores, "company_scores.csv")
```

---
title: "MVP Product Carbon TR Indicator"
output:
...
# B. Data
## Load ecoinvent and tilt company data
```{r}
here <- function(...) fs::path("~/Downloads/mvp/pctr", ...)
ecoinvent_co2 <- read_csv(here("02_input_data", "data_ecoinvent_co2.csv"))
ecoinvent_co2 |> glimpse()

companies <- read_csv(here("02_input_data", "tilt_companies.csv"))
companies |> glimpse()
```
...
## C.2. Calculate ranks per activity and assign scores
### C.2.1. Calculate the rank of each activity
...
ecoinvent_ranks |> glimpse()
```

### C.2.2. Assign scores to the activities
...
## C.3. Calculate scores per company
### C.3.1. Join dataset companies and ecoinvent_scores
...
companies_scores |> glimpse()
```

### C.3.2. Caclulate the scores for each company
...
scores_unit_sec |> glimpse()
```

### C.3.3. Create final dataset
